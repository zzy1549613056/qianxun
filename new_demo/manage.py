...
from apps.models import PostingsModel
...
def postingAdd():
    ...
    for page in range(pages):
        ...
        for id in new_id_dict:
            ...
            if content_match:
                post = PostingsModel(title=title_match.group(1),content=re.sub(r'(?<!com)\/Upload\/image\/', add, content_match.group(1)),board_id=6)
                db.session.add(post)
                db.session.commit()
                print(id)
                count += 1
